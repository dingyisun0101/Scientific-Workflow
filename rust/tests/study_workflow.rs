...
use scientific_workflow::study::advanced::{Study, StudyError};
use scientific_workflow::task::advanced::{ModelCatalog, ModelRegistration};
...
fn preflight_rejects_invalid_binding_without_output() {
    ...
    let bad_display = Project::new(
        r#"{"phases":{"only":{"tasks":[{"model":"counter","input":"inputs/counter.json","display":{"include":["absent"]}}]}}}"#,
        r#"{"initial":1,"steps":1}"#,
    );
    assert!(matches!(
        Study::load(bad_display.path()),
        Err(StudyError::UnknownDisplayField { field, .. }) if field == "absent"
    ));
    assert!(!bad_display.path().join("output").exists());
}

#[test]
fn explicit_catalog_is_deterministic_and_duplicate_checked() {
    let registration = ModelRegistration::new::<CounterModel>("counter");
    let catalog = ModelCatalog::from_registrations([registration]).unwrap();
    assert_eq!(catalog.keys().collect::<Vec<_>>(), ["counter"]);
    assert!(ModelCatalog::from_registrations([registration, registration]).is_err());

    let project = Project::new(
        r#"{"phases":{"only":{"tasks":[{"model":"counter","input":"inputs/counter.json"}]}}}"#,
        r#"{"initial":1,"steps":1}"#,
    );
    Study::load_with_catalog(project.path(), &catalog).unwrap();
}
