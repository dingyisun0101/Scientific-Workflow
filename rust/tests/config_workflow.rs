...
fn one_project_root_compiles_every_document_into_a_resolved_specification() {
    ...
    assert_eq!(
        specification.config_root(),
        specification.project_root().join("config")
    );
    assert_eq!(specification.documents().len(), 4);
    ...
    assert_eq!(replicates.base_seed(), Some(1101));
...
fn state_schema_is_parsed_once_by_config_then_semantically_validated_by_state() {
    ...
    assert_eq!(
        specification.documents()[1].bytes(),
        specification.state_schema().bytes()
    );
}
...
#[test]
fn central_advanced_prelude_reexports_the_config_boundary() {
    fn accepts(_: Option<scientific_workflow::prelude::advanced::ProjectSpecification>) {}
    fn accepts_input(_: Option<scientific_workflow::prelude::advanced::ResolvedTaskInput>) {}
    accepts(None);
    accepts_input(None);
}
