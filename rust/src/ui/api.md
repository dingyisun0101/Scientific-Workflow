...
Execution through `scientific_workflow::run(&Path)` or `runtime::execute(Study)`
requires terminal standard input and standard error. Crate composition checks
this before output creation or cleanup and returns a clear presentation error
instructing users to run inside `screen` or `tmux` when either stream is not a
terminal. There is no headless/plain execution mode or `terminal-ui` feature.
Configuration and reader APIs remain usable without executing a study.
...
- task paging with PageUp/PageDown; and
...
instead of scrolling the interactive terminal. Scientific payloads are never rendered. The dashboard is the required execution
interface; lifecycle history remains available in the durable log.
...
input, draw the dashboard, create/append/flush `log.txt`
...
Ratatui/Crossterm types, test-only observers, event variants,
...
glyphs, refresh interval, ETA formula, and cancellation
...
concurrent publishers, require a terminal dashboard, support
...
or resume is idempotent. A disk pause stays latched until usage reaches the
recovery threshold and the user types `resume` and Enter. Early commands are
rejected without clearing any pause or queuing future resumption. Both the
message panel and study status remind users; space recovery produces a second
reminder and never resumes automatically. Cancellation wakes paused participants. Force exit kills
...
scientific-time ETA because member sizes differ. Terminal
