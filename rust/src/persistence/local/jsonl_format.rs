//! Versioned metadata and record representations for local persistence.
//!
...
impl RecordingMetadata {
    ...
    /// commit.
    pub(crate) fn validate(&self, path: &Path) -> Result<(), PersistenceError> {
        ...
        for stream in &self.streams {
            if !names.insert(stream.name.as_str()) {
                return Err(PersistenceError::DuplicateStateStream {
...
impl RecordingTiming {
    ...
    fn validate(&self, path: &Path, status: &RecordingStatus) -> Result<(), PersistenceError> {
...
impl RecordingStatus {
    ...
    fn validate(&self, path: &Path) -> Result<(), PersistenceError> {
...
impl StateFieldMetadata {
    ...
    fn validate(&self, path: &Path, stream: &str) -> Result<(), PersistenceError> {
        if self.name.trim().is_empty() {
            return Err(invalid_metadata(
                path,
...
fn invalid_metadata(path: &Path, reason: impl Into<String>) -> PersistenceError {
