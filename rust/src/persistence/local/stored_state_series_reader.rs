...
//!
//! A read returns either the complete series or one [`PersistenceError`]. States
...
impl StoredStateSeriesReader {
    ...
    /// # Errors
    ///
    /// Returns [`PersistenceError::Io`] when `metadata.json` cannot be read,
    /// [`PersistenceError::Json`] when it is not syntactically valid JSON, semantic
    /// metadata errors from internal `RecordingMetadata::validate`, or
    /// [`PersistenceError::RecordingNotComplete`] unless status is complete.
    pub fn open_completed_recording(
        ...
        let bytes = fs::read(&metadata_path).map_err(|source| PersistenceError::Io {
        ...
        if !matches!(metadata.status, RecordingStatus::Complete) {
            return Err(PersistenceError::RecordingNotComplete {
                path: metadata_path,
            });
        }
    ...
    /// Returns the validated persistence format version.
    ...
    pub fn stream_record_count(&self, stream: &str) -> Result<u64, PersistenceError> {
        let declaration =
            self.metadata
                .stream(stream)
    ...
    pub fn stream_encoded_bytes(&self, stream: &str) -> Result<u64, PersistenceError> {
        let declaration =
            self.metadata
                .stream(stream)
                .ok_or_else(|| PersistenceError::UnknownStateStream {
    ...
    pub fn read_stream_as_state_series(
        &self,
        stream: &str,
    ) -> Result<StateSeries, PersistenceError> {
        let declaration =
            self.metadata
                .stream(stream)
                .ok_or_else(|| PersistenceError::UnknownStateStream {
        ...
        let capacity =
            usize::try_from(total_records).map_err(|_| PersistenceError::ByteCountOverflow {
                stream: declaration.name.clone(),
            })?;
    ...
    pub fn read_all_streams_as_state_series(
        &self,
    ) -> Result<Vec<(String, StateSeries)>, PersistenceError> {
        self.metadata
            .streams
            .iter()
            .map(|stream| {
    ...
    pub fn read_latest_state_from_stream(
        &self,
        stream: &str,
    ) -> Result<SystemState, PersistenceError> {
        let declaration =
            self.metadata
                .stream(stream)
                .ok_or_else(|| PersistenceError::UnknownStateStream {
        ...
        let chunk = declaration
            .chunks
            .last()
            .ok_or_else(|| PersistenceError::NoRecordedState {
                stream: declaration.name.clone(),
            })?;
    ...
    fn read_chunk(
        ...
        let file = File::open(&path).map_err(|source| {
            if source.kind() == std::io::ErrorKind::NotFound {
                PersistenceError::MissingChunk { path: path.clone() }
            } else {
                PersistenceError::Io {
        ...
        loop {
            ...
            *previous_iteration = Some(iteration);
            ...
            series.push_state(state).map_err(|rejection| {
                ...
                drop(state);
                PersistenceError::StateSeriesInvariant {
                    stream: stream.name.clone(),
                    iteration: index,
                    source,
                }
...
fn verify_file_size(path: &Path, expected: u64) -> Result<(), PersistenceError> {
    let actual = match fs::metadata(path) {
        Ok(metadata) => metadata.len(),
        Err(source) if source.kind() == std::io::ErrorKind::NotFound => {
            return Err(PersistenceError::MissingChunk {
    ...
    if actual != expected {
        return Err(PersistenceError::ChunkSizeMismatch {
            path: path.to_path_buf(),
            expected,
            actual,
        });
...
fn invalid_record(path: &Path, line: u64, reason: impl Into<String>) -> PersistenceError {
