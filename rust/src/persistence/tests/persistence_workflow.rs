//! Internal end-to-end local persistence and reconstruction tests.
//!
//! Run with:
//!
//! ```text
//! cargo test --lib persistence_workflow_tests -- --nocapture
//! ```
...
fn create_writer(
    ...
    let descriptor = BoundObservationPlan::bind(plan, schema).unwrap();
    SystemStateWriter::create(
        root,
        descriptor,
        metadata,
        StateStreamStorage::chunked(
            NonZeroU64::new(target_bytes).unwrap(),
...
fn complete_scientific_workflow_is_consistent_and_observable() {
    ...
    assert!(matches!(
        ObservationStream::all_fields("invalid")
            .unwrap()
            .every_iterations(0),
    ...
    let observation_plan = ObservationPlan::streams([
        ObservationStream::fields("signal", ["activity", "population"]).unwrap(),
        ObservationStream::fields("space", ["space"])
            .unwrap()
            .every_iterations(2)
            .unwrap(),
    ])
    ...
    output.record_final_state(&live).unwrap();
    output
        .complete_recording_with_terminal_metadata(terminal_metadata)
    ...
    assert!(
        space_metadata["chunks"]
            ...
            .iter()
            .all(|chunk| {
                chunk["records"] == 1 && chunk["bytes"].as_u64().unwrap() > CHUNK_BYTES
            })
    );
    ...
    create_writer(
        task_metadata_run.clone(),
        &spec,
        ObservationPlan::streams([ObservationStream::all_fields("checkpoint").unwrap()]).unwrap(),
        Map::from_iter([
    ...
    println!("[result] persistence_workflow=passed");
}
