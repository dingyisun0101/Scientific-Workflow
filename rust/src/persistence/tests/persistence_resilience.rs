//! Internal fault injection for persistence lifecycle and integrity.
//!
//! Run with:
//!
//! ```text
//! cargo test --lib persistence_resilience_tests -- --nocapture
//! ```
...
fn observation_plan(every_iterations: u64) -> ObservationPlan {
    ObservationPlan::streams([
        ObservationStream::fields("signal", ["population", "activity"])
            .unwrap()
...
fn create_recording(
    ...
    queue_bytes: u64,
) -> Result<SystemStateWriter, PersistenceError> {
    let descriptor = BoundObservationPlan::bind(plan, schema)
        .map_err(|source| PersistenceError::Observation { source })?;
    SystemStateWriter::create(
        root,
        descriptor,
        serde_json::Map::new(),
...
fn write_valid_run(workspace: &TempWorkspace) {
    let spec = spec();
    let mut output = create_recording(workspace.run(), &spec, observation_plan(1), 4_096).unwrap();
...
fn storage_failures_are_detected_with_context_and_without_partial_success() {
    ...
    assert!(matches!(
        create_recording(existing.run(), &state_spec, observation_plan(1), 4_096),
        Err(PersistenceError::RecordingDirectoryExists { path }) if path == existing.run()
    ));
    ...
    assert!(matches!(
        create_recording(
            invalid.run(),
            &state_spec,
            ObservationPlan::fields(["absent"]).unwrap(),
            4_096,
        ),
        Err(PersistenceError::Observation {
            source: ObservationError::UnknownField { field, .. }
        }) if field == "absent"
    ));
    ...
    assert!(matches!(
        ObservationPlan::streams([
            ObservationStream::all_fields("signal").unwrap(),
            ObservationStream::all_fields("signal").unwrap(),
        ]),
        Err(ObservationError::DuplicateStreamName { stream }) if stream == "signal"
    ));
    ...
    let mut output =
        create_recording(oversized.run(), &state_spec, observation_plan(1), 64).unwrap();
    ...
    assert!(matches!(
        ...
        Err(PersistenceError::RecordTooLarge { limit: 64, .. })
    ));
    output
    ...
    let mut output =
        create_recording(ordering.run(), &state_spec, observation_plan(1), 4_096).unwrap();
    ...
    assert!(matches!(
        ...
        Err(PersistenceError::Observation {
            source: ObservationError::NonIncreasingObservation {
                next: 4,
                previous: 5,
                ..
            }
    ...
    let mut output =
        create_recording(terminal.run(), &state_spec, observation_plan(1), 4_096).unwrap();
    ...
    assert!(matches!(
        ...
        Err(PersistenceError::StateWriterTerminated { .. })
    ));
    println!(
    ...
    let mut output =
        create_recording(encoding.run(), &state_spec, observation_plan(2), 4_096).unwrap();
    ...
    assert!(matches!(
        ...
        Err(PersistenceError::Observation {
            source: ObservationError::StateAccess { iteration: 4, .. }
        })
    ));
    ...
    assert!(matches!(
        &encode_error,
        PersistenceError::Observation {
            source: ObservationError::EncodeField {
                stream,
                iteration: 4,
                field,
                ..
    ...
    let output = create_recording(running.run(), &state_spec, observation_plan(1), 4_096).unwrap();
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&running.run(), decoders()),
        Err(PersistenceError::RecordingNotComplete { .. })
    ));
    ...
    assert!(matches!(
        ...
        Err(PersistenceError::UnknownStateStream { stream }) if stream == "absent"
    ));
    assert!(matches!(
        reader.read_stream_as_state_series("signal"),
        Err(PersistenceError::MissingDecoder { field }) if field == "population"
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&malformed.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::InvalidRecord { line: 1, .. })
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&missing_field.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::InvalidRecord { reason, .. }) if reason.contains("contains 1 payload values")
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&additional_field.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::InvalidRecord { reason, .. }) if reason.contains("contains 3 payload values")
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&missing.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::MissingChunk { path }) if path == missing_chunk
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&size.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::ChunkSizeMismatch { path, .. }) if path == size_chunk
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&checksum.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::ChecksumMismatch { path, .. }) if path == checksum_chunk
    ));
    ...
    assert!(matches!(
        StoredStateSeriesReader::open_completed_recording(&unterminated.run(), decoders())
            .unwrap()
            .read_stream_as_state_series("signal"),
        Err(PersistenceError::InvalidRecord { reason, .. }) if reason.contains("not terminated")
    ));
    ...
    println!("[result] persistence_resilience=passed");
}
