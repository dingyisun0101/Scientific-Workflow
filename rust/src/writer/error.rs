//! Writer declaration, observation, and encoding failures.

use thiserror::Error;

use crate::state::advanced::StateError;

/// A failure while defining, binding, or applying a scientific writer.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum WriterError {
    /// A multi-stream writer definition contained no streams.
    #[error("a writer must declare at least one observation stream")]
    EmptyWriter,

    /// A stream name normalized to an empty value.
    #[error("writer stream name must not be empty")]
    EmptyStreamName,

    /// Two streams used the same normalized scientific name.
    #[error("writer stream `{stream}` is declared more than once")]
    DuplicateStreamName {
    ...
    #[error("writer stream `{stream}` contains an empty field name")]
    EmptyFieldName {
    ...
    #[error("writer stream `{stream}` must select at least one state field")]
    EmptyFieldSelection {
    ...
    #[error("writer stream `{stream}` selects field `{field}` more than once")]
    DuplicateField {
    ...
    #[error("writer stream `{stream}` selects unknown state field `{field}`")]
    UnknownField {
    ...
    #[error("writer stream `{stream}` sampling interval must be greater than zero")]
    ...
    #[error("writer {axis} unit must not be empty")]
    EmptyAxisUnit {
    ...
    #[error("state at iteration {iteration} does not share the writer schema")]
    SchemaMismatch {
    ...
    #[error(
        "writer stream `{stream}` observed iteration {next} after accepted iteration {previous}"
    )]
